...
class WebSocketManager implements IWebSocketManager {
  ...
  public send(message: Partial<WebSocketMessage>): void {
    ...
    if (this.isConnected()) {
      try {
        ...
        console.log('WebSocket: Message sent', fullMessage.type, fullMessage.channel)
      } catch (error) {
        console.error('WebSocket: Failed to send message', error)
        this.messageQueue.push(fullMessage)
      }
    } else {
      console.log('WebSocket: Queuing message (not connected)', fullMessage.type)
      this.messageQueue.push(fullMessage)
    }
  }
  ...
  public isConnected(): boolean {
    return this.ws?.readyState === WebSocket.OPEN && this.connectionState.isConnected
  }
   ...
   */
  private messageMatchesFilters(message: WebSocketMessage, filters?: ChannelFilters): boolean {
   ...
   */
  private updateConnectionState(status: WebSocketStatus, error?: WebSocketError): void {
    this.connectionState = {
      ...this.connectionState,
      status,
  ...
  private scheduleReconnect(): void {
    ...
    console.log(`WebSocket: Scheduling reconnect attempt ${attempt}/${this.maxReconnectAttempts} in ${delay}ms`)
  ...
  private sendQueuedMessages(): void {
    while (this.messageQueue.length > 0) {
      ...
      try {
        ...
        console.log('WebSocket: Queued message sent', message.type, message.channel)
      } catch (error) {
