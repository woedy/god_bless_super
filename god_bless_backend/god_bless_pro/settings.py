...
# Email Configuration
# Default to console backend for local/dev setups so registration doesn't fail when no SMTP is configured.
...
STATICFILES_DIRS = [
    os.path.join(BASE_DIR, "static_cdn", "static_files"),
]
...
# HTTPS Settings (disabled for reverse proxy deployment)
# Coolify handles SSL termination, so we don't need Django to redirect
