...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  // Safe localStorage access with better default
  const [sidebarExpanded, setSidebarExpanded] = useState(() => {
    if (typeof window === 'undefined') return true; // Default to expanded for SSR
    try {
      const storedSidebarExpanded = localStorage.getItem('sidebar-expanded');
      // Default to true (expanded) if no stored value
      return storedSidebarExpanded === null ? true : storedSidebarExpanded === 'true';
    } catch (error) {
      console.warn('Error accessing localStorage for sidebar state:', error);
